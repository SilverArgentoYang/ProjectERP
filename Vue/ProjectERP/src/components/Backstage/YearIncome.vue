...
//侧边栏
store._backstage_sidebar(5,14);

//路径
store._addpath(1,'BYear','年度账单');
...
function _getdata(type) {
  axios({
    ...
    res.data.orders.forEach(item => {
      if(item.time[0]==new Date().getFullYear()){
...
<template>
  <div class="dailybody">
    ...
    <div class="banner">
      <div class="switch">
        全年收入：<span style="color: var(--colorred);font-weight: bold">￥{{totalincome}}</span>
      </div>
