...
//侧边栏
store._backstage_sidebar(5,12);

//路径
store._addpath(1,'BDaily','每日账单');
...
function _getdata(type) {
  axios({
    ...
    res.data.orders.forEach(item => {
      if(item.time[0]==new Date().getFullYear() && item.time[1]==new Date().getMonth()+1 && item.time[2]==new Date().getDate()){
...
<div class="dailybody">
  ...
  <div class="banner">
    <div class="switch">
      今日收入：<span style="color: var(--colorred);font-weight: bold">￥{{totalincome}}</span>
    </div>
